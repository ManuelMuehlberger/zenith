...
void main() {
  ...
  group('HomeScreen - Recent Workouts', () {
    ...
    testWidgets(
      'shows "Recent Workouts" and renders completed workouts from DB',
      (WidgetTester tester) async {
        ...
        ).thenAnswer((_) async => [inProgress, completed]);
        when(
    ...
    testWidgets('shows empty-state when no completed workouts exist', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('deleting from detail refreshes Home and removes workout', (
      WidgetTester tester,
    ) async {
      ...
      when(
        mockWorkoutDao.getAllWorkouts(),
      ).thenAnswer((_) async => currentWorkouts);
      when(
        mockWorkoutExerciseDao.getWorkoutExercisesByWorkoutIds(any),
      ).thenAnswer((_) async => []);
      when(
      ...
      when(
        mockWorkoutExerciseDao.getWorkoutExercisesByWorkoutId('w_completed'),
      ).thenAnswer((_) async => []);
      when(
        mockWorkoutExerciseDao.deleteWorkoutExercisesByWorkoutId('w_completed'),
      ).thenAnswer((_) async => 1);
      when(
        mockWorkoutDao.deleteWorkout('w_completed'),
      ).thenAnswer((_) async => 1);
