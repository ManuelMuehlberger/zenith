...
void main() {
  group('ReorderService', () {
    ...
    group('Drag Operations', () {
      ...
      test(
        'should only confirm the last drag start after multiple rapid starts',
        () {
          fakeAsync((async) {
            reorderService.onDragStarted(0);
    ...
    group('Drag Update and Timeout', () {
      ...
      test(
        'should not affect confirmation timer when updated before confirmation',
        () {
          fakeAsync((async) {
            reorderService.onDragStarted(0);
      ...
      test(
        'should refresh completion timeout when updated after confirmation',
        () {
          fakeAsync((async) {
            reorderService.onDragStarted(0);
    ...
    group('Reorder Completion', () {
      ...
      test(
        'should complete reorder and call WorkoutService when drag is confirmed',
        () {
          fakeAsync((async) {
            when(
              mockWorkoutService.reorderExercisesInWorkout(any, any, any),
            ).thenAnswer((_) async {});
            ...
            verify(
              mockWorkoutService.reorderExercisesInWorkout('workout123', 0, 2),
            ).called(1);
            expect(reorderService.draggingIndex, isNull);
            expect(reorderService.isAnyItemPotentiallyDragging, isFalse);
          });
    ...
    group('Helper Methods', () {
      ...
      test(
        'should correctly identify other items when one is being dragged',
        () {
          fakeAsync((async) {
            reorderService.onDragStarted(2);
            async.elapse(const Duration(milliseconds: 1001));
            ...
            expect(
              reorderService.isAnotherItemBeingActivelyDragged(2),
              isFalse,
            );
