...
void main() {
  ...
  group('WorkoutTemplateService Tests', () {
    ...
    test(
      'getAllWorkoutTemplates should return all templates ordered',
      () async {
        when(
    ...
    test(
      'getWorkoutTemplatesByFolder should return templates for a specific folder',
      () async {
        const folderId = 'folder123';
        final expected = testTemplates
            .where((t) => t.folderId == folderId)
            .toList();
        when(
          mockTemplateDao.getWorkoutTemplatesByFolderId(folderId),
    ...
    test(
      'getWorkoutTemplatesWithoutFolder should return templates with no folder',
      () async {
        final expected = testTemplates
            .where((t) => t.folderId == null)
            .toList();
        when(
          mockTemplateDao.getWorkoutTemplatesWithoutFolder(),
        ).thenAnswer((_) async => expected);
    ...
    test(
      'getWorkoutTemplateById should return a template when found',
      () async {
        when(
    ...
    test('getWorkoutTemplateById should return null when not found', () async {
      when(
        mockTemplateDao.getWorkoutTemplateById('not-found'),
    ...
    test(
      'createWorkoutTemplate should create and return a new template',
      () async {
    ...
    test('updateWorkoutTemplate should call the dao to update', () async {
      when(
        mockTemplateDao.updateWorkoutTemplate(testTemplate),
    ...
    test('deleteWorkoutTemplate should call the dao to delete', () async {
      when(
        mockTemplateDao.deleteWorkoutTemplate('template123'),
      ).thenAnswer((_) async => 1);
    ...
    test('markTemplateAsUsed should update the lastUsed timestamp', () async {
      ...
      verify(
        mockTemplateDao.updateLastUsed('template123', argThat(isA<String>())),
      ).called(1);
    });

    test(
      'getRecentlyUsedTemplates should return a list of templates',
      () async {
        when(
          mockTemplateDao.getWorkoutTemplatesByLastUsed(limit: 5),
        ).thenAnswer((_) async => testTemplates);
    ...
    test('moveTemplateToFolder should update the template folderId', () async {
      ...
      final verification = verify(
        mockTemplateDao.updateWorkoutTemplate(captureAny),
      );
    ...
    test(
      'getTemplateCountByFolder should return a map of folder counts',
      () async {
        when(
          mockTemplateDao.getAllWorkoutTemplatesOrdered(),
        ).thenAnswer((_) async => testTemplates);
        ...
        expect(result, {'folder123': 2, 'folder456': 1});
        verify(mockTemplateDao.getAllWorkoutTemplatesOrdered()).called(1);
      },
    );
    ...
    group('Folder Operations', () {
      ...
      test(
        'reorderTemplatesInFolder should reorder templates successfully',
        () async {
      ...
      test(
        'getTemplatesInFolder should return templates for a specific folder',
        () async {
          final templatesInFolder = [testTemplate];
          when(
            mockTemplateDao.getWorkoutTemplatesByFolderId('folder123'),
      ...
      test(
        'getTemplatesInFolder should return templates without folder when folderId is null',
        () async {
