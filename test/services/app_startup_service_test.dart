...
void main() {
  group('AppStartupService', () {
    test(
      'instance getter returns a reusable singleton with default wiring',
      () {
        final first = AppStartupService.instance;
        final second = AppStartupService.instance;
    ...
    test(
      'clears cached future after failure so initialization can retry',
      () async {
        var shouldFail = true;
        var initializeNotificationsCalls = 0;
        var loadExercisesCalls = 0;
    ...
    test(
      'reuses successful initialization for later callers without rerunning startup',
      () async {
        var initializeNotificationsCalls = 0;
        var initializeNotificationCallbackCalls = 0;
        var loadExercisesCalls = 0;
    ...
    test(
      'loads active session only after notification setup and parallel data loads finish',
      () async {
        final events = <String>[];
    ...
    test(
      'retries after notification callback failure before loading app data',
      () async {
        var shouldFail = true;
        var initializeNotificationsCalls = 0;
        var initializeNotificationCallbackCalls = 0;
        ...
        expect(initializeNotificationsCalls, 1);
        expect(initializeNotificationCallbackCalls, 1);
        expect(loadExercisesCalls, 0);
        expect(loadWorkoutDataCalls, 0);
        expect(loadUserProfileCalls, 0);
        expect(loadActiveSessionCalls, 0);

        shouldFail = false;
        await service.initializeMainApp();

        expect(initializeNotificationsCalls, 2);
        expect(initializeNotificationCallbackCalls, 2);
        expect(loadExercisesCalls, 1);
        expect(loadWorkoutDataCalls, 1);
        expect(loadUserProfileCalls, 1);
        expect(loadActiveSessionCalls, 1);
    ...
    test('retries full startup when loading active session fails', () async {
      var shouldFail = true;
      var initializeNotificationsCalls = 0;
      var initializeNotificationCallbackCalls = 0;
      var loadExercisesCalls = 0;
      ...
      final service = AppStartupService(
        ...
        loadActiveSession: () async {
          loadActiveSessionCalls += 1;
          if (shouldFail) {
            throw StateError('active session failed');
          }
        },
      );
