  ...
  group('ExerciseService Tests', () {
    ...
    group('loadExercises', () {
      test('should load exercises and muscle groups successfully', () async {
        ...
        final sortedNames = testMuscleGroups.map((mg) => mg.name).toList()
          ..sort();
      ...
      test('should handle exception when loading exercises', () async {
        ...
        expect(exerciseService.exercises, isEmpty);
        expect(
          exerciseService.allMuscleGroups,
          hasLength(7),
        ); // Muscle groups should still load
      });
      ...
      test('should handle exception when loading muscle groups', () async {
        ...
        expect(
          ...
          hasLength(4),
        ); // Exercises should still load
        expect(
          exerciseService.allMuscleGroups,
          isEmpty,
        ); // Only muscle groups should be empty
      });
      ...
      test('should handle exception when both DAOs fail', () async {
        ...
        when(
          mockExerciseDao.getAllExercises(),
        ).thenThrow(Exception('Exercise DAO error'));
        when(
          mockMuscleGroupDao.getAllMuscleGroups(),
        ).thenThrow(Exception('MuscleGroup DAO error'));
    ...
    group('searchExercises', () {
      ...
      test('should filter exercises by primary muscle group', () {
        ...
        expect(
          result.every((e) => e.primaryMuscleGroup == MuscleGroup.chest),
          isTrue,
        );
        expect(
          result.map((e) => e.name),
      ...
      test('should handle case-insensitive search for muscle group', () {
        ...
        expect(
          result1.every((e) => e.primaryMuscleGroup == MuscleGroup.chest),
          isTrue,
        );
        expect(
          result2.every((e) => e.primaryMuscleGroup == MuscleGroup.chest),
          isTrue,
        );
        expect(
          result3.every((e) => e.primaryMuscleGroup == MuscleGroup.chest),
          isTrue,
        );
      });
      ...
      test('should handle whitespace in queries', () {
        ...
        expect(
          result2,
          hasLength(4),
        ); // Empty query after trim should return all
      });
      ...
      test('should search by secondary muscle groups', () {
        ...
        expect(
          result.every(
            (e) => e.secondaryMuscleGroups.contains(MuscleGroup.triceps),
          ),
          isTrue,
        );
      });
    ...
    group('filterByMuscleGroup', () {
      ...
      test(
        'should filter exercises by primary muscle group (case insensitive)',
        () {
          // Act
    ...
    group('allMuscleGroups getter', () {
      ...
      test('should return sorted muscle group names after loading', () async {
        ...
        // Verify sorting
        final expectedSorted = testMuscleGroups.map((mg) => mg.name).toList()
          ..sort();
      ...
      test(
        'should maintain sorted order with duplicate muscle groups',
        () async {
          ...
          when(
            mockMuscleGroupDao.getAllMuscleGroups(),
          ).thenAnswer((_) async => duplicateMuscleGroups);
    ...
    group('exercises getter', () {
      ...
      test('should return loaded exercises', () async {
        ...
        expect(
          result.map((e) => e.name),
          containsAll(['Bench Press', 'Push Up', 'Squat', 'Deadlift']),
        );
      });
    ...
    group('edge cases and error handling', () {
      test('should handle null or malformed data gracefully', () async {
        ...
        expect(
          () => exerciseService.filterByMuscleGroup('test'),
          returnsNormally,
        );
      });
      ...
      test('should handle very long search queries', () async {
        ...
        expect(result, isEmpty);
        expect(
          () => exerciseService.searchExercises(longQuery),
          returnsNormally,
        );
      });
      ...
      test('should handle special characters in search queries', () async {
        ...
        expect(
          () => exerciseService.searchExercises('weightlifter'),
          returnsNormally,
        );
        expect(
    ...
    group('performance and behavior tests', () {
      test('should handle large datasets efficiently', () async {
        ...
        final largeExerciseList = List.generate(
          1000,
          (index) => Exercise(
            slug: 'exercise-$index',
            name: 'Exercise $index',
            primaryMuscleGroup:
                MuscleGroup.values[index % MuscleGroup.values.length],
        ...
        when(
          mockExerciseDao.getAllExercises(),
        ).thenAnswer((_) async => largeExerciseList);
        when(
          mockMuscleGroupDao.getAllMuscleGroups(),
        ).thenAnswer((_) async => testMuscleGroups);
      ...
      test('should return immutable results from search and filter', () async {
        ...
        expect(
          exerciseService.searchExercises('Chest'),
          hasLength(2),
        ); // Should still work
        expect(
          exerciseService.filterByMuscleGroup('Chest'),
          hasLength(2),
        ); // Should still work
      });
    });
