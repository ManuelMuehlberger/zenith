...
void main() {
  group('WeeklyInsights', () {
    ...
    test(
      'getWeeklyInsights calculates workout counts correctly for 6 months',
      () async {
    ...
    test('getWeeklyInsights uses caching', () async {
      ...
      expect(
        insights1.lastUpdated.millisecondsSinceEpoch,
        insights2.lastUpdated.millisecondsSinceEpoch,
      );
    });
    ...
    test('getWeeklyInsights handles week boundaries correctly', () async {
      ...
      final workouts = [
        ...
        _createWorkout(
          ...
          45,
          15,
        ), // Sunday Jan 12 (same week)
        _createWorkout(
          monday.subtract(const Duration(days: 7)),
          50,
          25,
        ), // Previous Monday (Dec 30)
      ];
    ...
    test(
      'getWeeklyInsights uses monthly grouping for 1 year timeframe',
      () async {
        final now = DateTime(2025, 1, 10, 12, 0);
        final workouts = [
    ...
    test(
      'getWeeklyInsights uses monthly grouping for 2 year timeframe',
      () async {
        final now = DateTime(2025, 1, 10, 12, 0);
        final workouts = [
          // This month: 2 workouts
    ...
    group('Filtering', () {
      test('filter by workout name', () async {
        ...
        final workouts = [
          _createWorkout(
            ...
            60,
            20,
          ).copyWith(name: 'Leg Day'),
          _createWorkout(
            now.subtract(const Duration(days: 2)),
            45,
            15,
          ).copyWith(name: 'Push Day'),
          _createWorkout(
            now.subtract(const Duration(days: 3)),
            50,
            25,
      ...
      test('filter by muscle group', () async {
        ...
        final workout1 = _createWorkout(
          now.subtract(const Duration(days: 1)),
          60,
          20,
        );
        // Manually update exercise slugs to match our seeded exercises
        final w1Exercises = workout1.exercises
            .map((e) => e.copyWith(exerciseSlug: 'bench-press'))
            .toList();
        final w1 = workout1.copyWith(exercises: w1Exercises);

        // Workout 2: Legs only (15 sets)
        final workout2 = _createWorkout(
          now.subtract(const Duration(days: 2)),
          45,
          15,
        );
        final w2Exercises = workout2.exercises
            .map((e) => e.copyWith(exerciseSlug: 'squat'))
            .toList();
        final w2 = workout2.copyWith(exercises: w2Exercises);

        // Workout 3: Mixed (10 sets chest, 10 sets legs)
        final workout3 = _createWorkout(
          now.subtract(const Duration(days: 3)),
          90,
          20,
        );
      ...
      test('filter by equipment', () async {
        ...
        final w1 = workout1.copyWith(
          exercises: workout1.exercises
              .map((e) => e.copyWith(exerciseSlug: 'db-press'))
              .toList(),
        );
        ...
        final w2 = workout2.copyWith(
          exercises: workout2.exercises
              .map((e) => e.copyWith(exerciseSlug: 'bb-press'))
              .toList(),
        );
      ...
      test('filter by bodyweight', () async {
        ...
        final w1 = workout1.copyWith(
          exercises: workout1.exercises
              .map((e) => e.copyWith(exerciseSlug: 'pushups'))
              .toList(),
        );
