...
class CustomReorderableExerciseList extends StatefulWidget {
  ...
  @override
  State<CustomReorderableExerciseList> createState() =>
      _CustomReorderableExerciseListState();
}

class _CustomReorderableExerciseListState
    extends State<CustomReorderableExerciseList> {
  ...
  Widget _buildDropZone(BuildContext context, int dropZoneIndex) {
    ...
    return DragTarget<int>(
      onWillAcceptWithDetails: (details) {
        ...
        if (draggedItemActualIndex == dropZoneIndex ||
            draggedItemActualIndex == dropZoneIndex - 1) {
      ...
      onAcceptWithDetails: (details) {
        final draggedItemActualIndex =
            details.data; // This is the original index of the dragged item
        ...
        final newActualIndex = dropZoneIndex;
      ...
      builder: (context, candidateData, rejectedData) {
        final bool isHovering =
            _hoveredDropZoneIndex == dropZoneIndex && candidateData.isNotEmpty;
        final bool isDraggingInProgress = _draggedActualIndex != null;

        if (!isDraggingInProgress && !isHovering) {
          return const SizedBox.shrink(); // Only show if dragging or specifically hovered
        }
        ...
        return AnimatedContainer(
          ...
          height: isHovering
              ...
              : (isDraggingInProgress
                    ? 12
                    : 0), // Smaller when not hovered but dragging
          margin: EdgeInsets.symmetric(
            vertical: isHovering ? 6.0 : (isDraggingInProgress ? 2.0 : 0),
            horizontal:
                (widget.padding?.horizontal ?? 32.0) /
                2, // Use half of list padding
          ),
  ...
  Widget _buildDraggableItem(BuildContext context, int itemActualIndex) {
    ...
    return LongPressDraggable<int>(
        ...
        builder: (context) {
          ...
          return Material(
            ...
            child: SizedBox(
              ...
              height:
                  widget.itemExtent *
                  0.7, // Reduced height to match visual consistency
              child: Opacity(
                opacity: 0.95,
      ...
      childWhenDragging:
          const SizedBox.shrink(), // Completely hide the original item when dragging
      onDragStarted: () {
      ...
      onDragEnd: (_) {
        // Called when drag ends, regardless of acceptance
        _stopAutoScroll();
      ...
      onDragCompleted: () {
        // Called ONLY if accepted by a DragTarget
        _stopAutoScroll();
      ...
      child: AnimatedOpacity(
        // Apply opacity if this item is being dragged
        opacity: isSelfDragging
            ? 0.5
            : (_draggedActualIndex != null
                  ? 0.6
                  : 1.0), // Gray out other items when any item is being dragged
        duration: const Duration(milliseconds: 200),
        child: widget.itemBuilder(
          context,
          itemActualIndex,
