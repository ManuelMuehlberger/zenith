...
class WorkoutSessionService extends ChangeNotifier {
  ...
  NotificationServiceAPI _notificationService =
      LiveWorkoutNotificationService();
  @visibleForTesting
  set notificationService(NotificationServiceAPI service) =>
  ...
  bool get hasActiveSession {
    ...
    return session != null && session.status == WorkoutStatus.inProgress;
  }

  Future<void> _updateNotificationIfNeeded(Workout session) async {
  ...
  @visibleForTesting
  Future<void> enrichExercisesWithDetails(
  ...
  Future<void> loadActiveSession() async {
    ...
    try {
      ...
      if (inProgressWorkouts.isNotEmpty) {
        final loadedSession = inProgressWorkouts.first;
        _currentSession = loadedSession;
        _logger.fine('Found active session with id: ${loadedSession.id}');

        final workoutExercises = await _workoutExerciseDao
            .getWorkoutExercisesByWorkoutId(loadedSession.id);
        ...
        final hydratedSession = loadedSession.copyWith(
          exercises: exercisesWithSets,
        );
        _currentSession = hydratedSession;
  ...
  Future<Workout> startWorkout(Workout workout) async {
    ...
    for (int i = 0; i < workout.exercises.length; i++) {
      ...
      final Workout? lastWorkout = await WorkoutService.instance
          .getLastWorkoutForExercise(exerciseSlug);
      ...
      final sourceExercise = lastWorkout != null
          ? templateExercise.copyWith(
              sets: List<WorkoutSet>.from(templateExercise.sets),
            )
          : templateExercise;

      // Clone to session-scoped exercise with fresh IDs and proper foreign keys
      final cloned = cloneTemplateExerciseForSession(
        sourceExercise,
        sessionId,
        i,
      );
    ...
    _logger.fine('Saved new session to database with id: ${session.id}');
  ...
  Future<void> nextSet() async {
    ...
    final currentExercise = session.exercises[_currentExerciseIndex];
    if (_currentSetIndex < currentExercise.sets.length) {
      ...
      if (updatedSession == null) {
        _logger.warning('Session was cleared while advancing to next set');
        return;
      }
      ...
      if (_currentSetIndex < currentExercise.sets.length - 1) {
        ...
        if (_currentExerciseIndex < updatedSession.exercises.length - 1) {
          _currentExerciseIndex++;
          _currentSetIndex = 0;
        } else {}
      }

      await _workoutDao.updateWorkout(updatedSession);
      await _updateNotificationIfNeeded(updatedSession);
    }
    notifyListeners();
  }
  ...
  Future<void> previousSet() async {
    ...
    if (_currentSetIndex > 0) {
      ...
      if (_currentExerciseIndex > 0) {
        _currentExerciseIndex--;
        final previousExercise = session.exercises[_currentExerciseIndex];
        _currentSetIndex = previousExercise.sets.isNotEmpty
  ...
  Future<void> updateSet(
    ...
    final exerciseIdx = session.exercises.indexWhere((e) => e.id == exerciseId);
    if (exerciseIdx == -1) {
      _logger.warning('Exercise with id $exerciseId not found');
      return;
    }

    final exercise = session.exercises[exerciseIdx];
    ...
    _logger.finer(
      'Set ${updatedSet.id} updated from ${currentSet.toMap()} to ${updatedSet.toMap()}',
    );
    ...
    final updatedExercises = List<WorkoutExercise>.from(session.exercises);
  ...
  Future<Workout> completeWorkout({
    ...
    final startTime = session.startedAt ?? now;
    ...
    _logger.fine(
      'Completing workout: start=$startTime, '
  ...
  Future<void> clearActiveSession({bool deleteFromDb = false}) async {
    ...
    final session = _currentSession;
    if (deleteFromDb && session != null) {
      final workoutId = session.id;
      _logger.info(
        'Deleting workout and all associated data from database: $workoutId',
      );
      ...
      final exercisesToDelete = await _workoutExerciseDao
      ...
      for (final exercise in exercisesToDelete) {
        ...
        final setsToDelete = await _workoutSetDao
      ...
      _logger.info(
        'Workout deletion process complete for workout ID: $workoutId',
      );
    }
