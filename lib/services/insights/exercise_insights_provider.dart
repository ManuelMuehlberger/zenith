...
class ExerciseInsightsProvider {
  Future<ExerciseInsights> getData({
    ...
    final totalSets = recentInstances.fold<int>(
      0,
      (sum, instance) => sum + instance.totalSets,
    );
    final totalReps = recentInstances.fold<int>(
      0,
      (sum, instance) => sum + instance.totalReps,
    );
    final totalWeight = recentInstances.fold<double>(
      0,
      (sum, instance) => sum + instance.totalWeight,
    );
    final maxWeight = recentInstances.isEmpty
        ? 0.0
        : recentInstances
              .map((i) => i.maxWeight)
              .reduce((a, b) => a > b ? a : b);
  ...
  Map<String, List<InsightDataPoint>> _calculateExerciseTrendData(
    ...
    for (int i = slots - 1; i >= 0; i--) {
      ...
      final volume = slotInstances.fold<double>(
        0,
        (sum, instance) => sum + instance.totalWeight,
      );
      ...
      trendVolume.add(InsightDataPoint(date: slotStart, value: volume));

      trendMaxWeight.add(
        InsightDataPoint(date: slotStart, value: currentMaxWeight),
      );

      trendFrequency.add(InsightDataPoint(date: slotStart, value: frequency));
    }
