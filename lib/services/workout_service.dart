...
class WorkoutService extends ChangeNotifier {
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      final workoutIds = _workouts
          .map((workout) => workout.id)
          .toList(growable: false);
      final workoutExercises = await _workoutExerciseDao
          .getWorkoutExercisesByWorkoutIds(workoutIds);
      ...
      for (final workoutSet in workoutSets) {
        setsByExerciseId
            .putIfAbsent(workoutSet.workoutExerciseId, () => [])
            .add(workoutSet);
      }
      ...
      for (final workoutExercise in workoutExercises) {
        ...
        final exerciseWithSets = workoutExercise.copyWith(
          sets: setsByExerciseId[workoutExercise.id] ?? const <WorkoutSet>[],
        );
        exercisesByWorkoutId
            .putIfAbsent(workoutId, () => [])
            .add(exerciseWithSets);
      }
  ...
  Future<int> rebuildAchievementsForCompletedWorkouts() async {
    _logger.info('Rebuilding achievements for completed workout history');
    ...
    final priorCompletedHistory = <Workout>[];
    var rebuiltCount = 0;
    ...
    for (final workout in completedWorkouts) {
      ...
      final achievements = await _workoutAchievementService.evaluateForWorkout(
        workout,
        history: priorCompletedHistory,
      ...
      priorCompletedHistory.add(workout.copyWith(achievements: achievements));
      rebuiltCount++;
    }

    await loadData();
    _logger.info('Rebuilt achievements for $rebuiltCount completed workouts');
    return rebuiltCount;
  }
  ...
  Future<void> reorderWorkoutsInFolder(
    String? folderId,
    int oldIndex,
    int newIndex,
  ) async {
    _logger.info(
      'Reordering workouts in folder $folderId from $oldIndex to $newIndex',
    );
    final workoutsInFolder = getWorkoutsInFolder(folderId);
    if (oldIndex < 0 ||
        oldIndex >= workoutsInFolder.length ||
        newIndex < 0 ||
        newIndex >= workoutsInFolder.length) {
  ...
  Future<void> reorderExercisesInWorkout(
    String workoutId,
    int oldIndex,
    int newIndex,
  ) async {
    _logger.info(
      'Reordering exercises in workout $workoutId from $oldIndex to $newIndex',
    );
  ...
  Future<void> addExerciseToWorkout(
    ...
    _logger.info(
      'Adding exercise ${exerciseDetail.slug} to workout $workoutId',
    );
    final workoutIndex = _workouts.indexWhere((w) => w.id == workoutId);
    if (workoutIndex != -1) {
      ...
      final updatedExercises = List<WorkoutExercise>.from(
        _workouts[workoutIndex].exercises,
      )..add(exerciseWithSet);
  ...
  Future<void> updateWorkoutExercise(
    ...
    _logger.fine(
      'Updating workout exercise ${exercise.id} in workout $workoutId',
    );
    final workoutIndex = _workouts.indexWhere((w) => w.id == workoutId);
    if (workoutIndex != -1) {
  ...
  Future<void> addSetToExercise(
    ...
    final workoutIndex = _workouts.indexWhere((w) => w.id == workoutId);
    if (workoutIndex != -1) {
      final exerciseIndex = _workouts[workoutIndex].exercises.indexWhere(
        (e) => e.id == exerciseId,
      );
      if (exerciseIndex != -1) {
  ...
  Future<void> removeSetFromExercise(
    ...
    _logger.info(
      'Removing set $setId from exercise $exerciseId in workout $workoutId',
    );
    final workoutIndex = _workouts.indexWhere((w) => w.id == workoutId);
    if (workoutIndex != -1) {
      final exerciseIndex = _workouts[workoutIndex].exercises.indexWhere(
